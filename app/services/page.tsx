...
export default function PricingPage() {
  return (
    <div className="container mx-auto px-4 py-12 max-w-7xl">
      ...
      <Tabs defaultValue="product-photography" className="w-full">
        <TabsList className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 w-full mb-8">
          <TabsTrigger value="product-photography" className="flex flex-col items-center gap-2 py-3">
            <Camera className="h-5 w-5" />
            <span>Product Photography</span>
          </TabsTrigger>
          <TabsTrigger value="product-videos" className="flex flex-col items-center gap-2 py-3">
            <Video className="h-5 w-5" />
            <span>Product Videos</span>
          </TabsTrigger>
          <TabsTrigger value="short-form" className="flex flex-col items-center gap-2 py-3">
            <Smartphone className="h-5 w-5" />
            <span>Short-Form Content</span>
          </TabsTrigger>
          <TabsTrigger value="cinematography" className="flex flex-col items-center gap-2 py-3">
            <Film className="h-5 w-5" />
            <span>Cinematography</span>
          </TabsTrigger>
          <TabsTrigger value="brand-ads" className="flex flex-col items-center gap-2 py-3">
            <Palette className="h-5 w-5" />
            <span>Brand Spec Ads</span>
          </TabsTrigger>
          <TabsTrigger value="web-branding" className="flex flex-col items-center gap-2 py-3">
        ...
        <TabsContent value="cinematography">
          <div className="grid md:grid-cols-3 gap-6">
            ...
            <PricingCard
              ...
              features={[
                "8-hour shoot",
                "Multi-location + lighting setups",
                "Raw footage + basic edit preview"
              ]}
            />
            <PricingCard
...
function PricingCard({ title, price, description, features, popular, className }: PricingCardProps) {
  return (
    <Card className={`relative ${popular ? 'border-primary shadow-lg' : ''} ${className}`}>
      {popular && (
        <Badge className="absolute top-0 right-0 translate-x-1/3 -translate-y-1/3 px-3 py-1">
          Popular
        </Badge>
      )}
      <CardHeader>
